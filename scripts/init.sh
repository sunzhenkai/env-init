...
# 需要设置
function ExportEnvs() {
    [ -z "$NEW_HOME" ] && export NEW_HOME="$HOME"
    export ENV_INIT_DIR="$NEW_HOME/.local/env-init"
    ...
    export ENV_INIT_BASE="$_BASE"
    export ENV_INIT_BIN="$_BASE/bin"
    export PATH="$ENV_INIT_BIN":$PATH
}
...
function EnvInit() {
    ...
    tool::append_to_env_profile "export ENV_INIT_BASE=$_BASE"
